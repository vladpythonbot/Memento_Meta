...
def app_links_keyboard(webapp_url: str | None = None) -> InlineKeyboardMarkup | None:
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="Открыть панель", web_app=WebAppInfo(url=f"{webapp_url}/app?v=4.8"))],
        ]
    )
...
def matrix_tasks_keyboard(tasks, webapp_url: str | None = None) -> InlineKeyboardMarkup | None:
    ...
    if webapp_url:
        rows.append([InlineKeyboardButton(text="Открыть матрицу", web_app=WebAppInfo(url=f"{webapp_url}/matrix?v=4.8"))])
...
def focus_methods_keyboard(webapp_url: str | None = None) -> InlineKeyboardMarkup:
    ...
    if webapp_url:
        rows.append([InlineKeyboardButton(text="Открыть фокус", web_app=WebAppInfo(url=f"{webapp_url}/focus?v=4.8"))])
